...
From our experience, the step from the region-based networks to one-stage detectors can be hard to comprehend. To bridge the gap between the two approaches, this section offers a thorough explanation of one-stage detector building process. 

At this point we do not concern ourselves with optimal model but rather present general ideas.

\subsection*{Detection Input}
Similarly to \cref{fig:fasterrcnn}, we start by processing the input image into a feature map. This process can be done by any extraction network, e.g., a classification network up to the fully connected layer, that outputs a feature map of [n\x n\x c] size. 
To perform a detection, we first need a 
Lets start by defining an input of such model. The ussual input for detection is a RGB image with [m\x m\x 3] shape (for convenience we will work only with square inputs). To keep in line with tied and tested methods, this image gets processed by a feature  We will build our detector on this feature map.

\subsection*{Desired Output}
To perform a bounding-box localization, we first need to parametrize the bounding-box. Since bbox is a rectangle, it can be specified by four parameters: a center position \textit{(X,Y)}, width \textit{W} and height \textit{H}. These can be either relative values in respect to predefined anchor boxes, or a absolute values. 

A predicted bbox also needs to be classified. To perform a standard soft-max classification, we require a confidence score for each of \textit{C} classes. However, the networks also needs to be able to classify the bbox as a background. This can be achieved by additional parameter with object/background confidence score \textit{B}.

We ended up with a set of C+5 parameters [c$_1$, ..., c$_C$, B, X, Y, W, H]. Were we in the region-based network, this is everything needed for one region. However, there are no given regions in one-stage detector, or should we say, every position is a region. Given the feature map [n\x n\x c], we expect to predict described vector of C+5 parameters for every cell of [n\x n] map. This gives us an expected tensor of [n\x n\x C+5] size. 

\subsubsection{Anchor Boxes}
In \cref{fig:fasterrcnn}, we saw the use of multiple anchor boxes in region proposal network. Experiments show that it is easier for the network to accept a set of suggested boxes and refine their position and scale, rather than regress values by itself. It is also better for the generalization if the network can associate boxes with certain aspect rations with certain classes.

There are multiple options to include this aspect in our model. We can either predict \textit{K} bounding-box parameters and one set of confidence scores per each region, or to predict a separate classification for each box. In the first case we get the prediction tensor of [n\x n\x K*4+C+1] shape and in the latter case of [n\x n\x K*(C+5)] shape.

\subsection*{Detection}

\subsubsection{Fully Connected Classifier}

\subsubsection{Convolutional Classifier}
