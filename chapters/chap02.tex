...
\label{sec:notation}
To avoid any confusion, we define the standard notation used though-out this thesis. We tried to follow the convention, but there can be some variance compared to other works.
...
We will also be using 3d convolutional layers, that produce five dimensional tensors. However, we will not add this dimension \textit{d} to the end of the list, but instead keep the batch size at the last place \textbf{[a\x b\x c\x d\x n]}. 

We ordered the values to allow for unambiguous reference o data without listing every dimension. For example, all data passing between two-dimensional convolutional layers is in form of four dimensional tensors, but we will often use only spatial dimensions or spacial and channel dimension to describe given tensor.

\subsubsection*{Convolution}
Since convolutional layers are chained together, and always work with the data with \textit{channel} dimension, we do not need to explicitly state the channel depth of the input data. We know that if previous convolution outputs data with the size of [a\x b\x c], and we want to apply \textit{k\x k} kernel, the actual size of the kernel is [k\x k\x c]. 

Unless stated otherwise, all convolutional layers use padding and stride of one.

\paragraph{Conv. k$_1$\x k$_2$\x c} : 2d convolutional layer with kernel size \textit{k$_1$\x k$_2$} and output channel depth of \textit{c}. The input channel depth is given by the output size of the previous layer. In simple terms, the layer accepts the input of [a\x b\x c$_\text{previous}$] input and a\x b times applies k$_1$\x k$_2$ \x c$_\text{previous}$ kernel \textit{c} times, leading to [a\x b\x c] output.

\paragraph{Conv. k$_1$\x k$_2$\x c /s} : 2d convolutional layer with stride \textit{s}.

\paragraph{Conv. k$_1$\x k$_2$\x k$_3$\x c} : 3d convolutional layer with kernel size \textit{k$_1$\x k$_2$ \x k$_3$} and output channel depth of \textit{c}. The input channel depth is given by the output size of the previous layer. In simple terms, the layer accepts the input of [a\x b\x d\x c$_\text{previous}$] input and a\x b\x d times applies k$_1$\x k$_2$\x k$_3$\x c$_\text{previous}$ kernel \textit{c} times, leading to [a\x b\x c] output.
...
\textbf{Max-pooling k$_1$\x k$_2$ /s} : maximum pooling with kernel size \textit{k$_1$\x k$_2$} and stride \textit{s}.
